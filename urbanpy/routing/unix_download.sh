...
mkdir -p ~/data/osrm/;
cd ~/data/osrm/;
# container 1 country 2 continent 3
wget https://download.geofabrik.de/$3/$2-latest.osm.pbf;
docker run -t --name osrm_extract -v $(pwd):/data osrm/osrm-backend osrm-extract -p /opt/foot.lua /data/$2-latest.osm.pbf;
...
docker run -t --name $1_$3_$2 -p 5000:5000 -v $(pwd):/data osrm/osrm-backend osrm-routed --algorithm mld /data/$2-latest.osm.pbf;